...
//var_dump($members);
$borrowed_list= $b_model->listMembersBooks($logged_in_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($getvars["action"]) && $getvars["action"] == 'update') {
        ...
        $return_date = date('Y-m-d H:i:s'); // Format: "YYYY-MM-DD HH:MM:SS"
        $status = "Returned";

        $added_return= $b_model->add_return($members_id, $books_id, $return_date);
...
include '../views/returnbook_view.php'
